...
function preprocessBrandData(brandName) {
    ...
    const subcategoriesFound = []; // ⭐ NEW: Track all subcategories
    const codeMap = new Map();
    const barcodeMap = new Map();
    const generatedBarcodeMap = new Map(); // ⭐ Track generated barcodes globally
    ...
    for (const row of rawData) {
        ...
        if (itemType && currentSubcategory) {
            ...
            // ⭐ Track ItemType even if no products follow
            itemTypesFound.push({
                category: currentCategory,
                subcategory: currentSubcategory,
    ...
    // ⭐ Find duplicate BARCODES - set barcode to null, keep product
...
async function seedBrand(brandName, validationData) {
    ...
    const { validProducts, itemTypesFound, subcategoriesFound } = validationData; // ⭐ Add subcategoriesFound

    let totalInserted = 0;
    let errors = [];
    let barcodeConflicts = [];
    let emptyItemTypesCreated = [];
    let emptySubcategoriesCreated = []; // ⭐ NEW
            ...
            for (const [itemName, products] of Object.entries(itemTypes)) {
                ...
                for (const p of products) {
                    try {
                        ...
                        // ⭐ If barcode is null, keep it null (no generation)

                        const product = await prisma.product.create({
                            data: {
                                code: p.code,
                                barcode: barcodeToUse, // ⭐ Can be null
                                nameAr: p.name,
                                nameEn: '',
                                categoryId: categoryObj.id,
    ...
    if (barcodeConflicts.length > 0) {
        console.log(`  ⚠️ Barcode conflicts (cross-brand): ${barcodeConflicts.length} (added DUPL suffix)`);
    }
    if (emptySubcategoriesCreated.length > 0) {
        console.log(`  📁 Empty Subcategories created: ${emptySubcategoriesCreated.length}`);
    }
    if (emptyItemTypesCreated.length > 0) {
        console.log(`  📁 Empty ItemTypes created: ${emptyItemTypesCreated.length}`);
    }
    if (errors.length > 0) {
...
function generateComprehensiveReport(brandName, validationData, seedResult) {
    ...
    const {
        ...
        productsWithNullifiedBarcodes,
        autoGeneratedCodes,
        autoGeneratedBarcodes, // ⭐ Changed from emptyBarcodes
        itemTypesFound
    } = validationData;
    ...
    return {
        summary: {
            ...
            autoGeneratedBarcodes: autoGeneratedBarcodes.length, // ⭐ Changed
        ...
        autoGeneratedBarcodes: autoGeneratedBarcodes, // ⭐ Changed from emptyBarcodes
...
function exportTextReport(brandName, reportData) {
    ...
    output += `✅ Successfully Inserted:                        ${reportData.summary.inserted}\n`;
    output += `❌ Insertion Errors:                             ${reportData.summary.errors}\n`;
    output += `❌ Skipped (No Name):                            ${reportData.summary.skippedNoName}\n`;
    output += `❌ Removed (Duplicate Codes in Excel):           ${reportData.summary.duplicateCodesRemoved} products\n`;
    output += `⭐ Auto-Generated Codes:                         ${reportData.summary.codesAutoGenerated} products\n`;
    output += `⭐ Barcodes Set to NULL (Duplicate in Excel):    ${reportData.summary.duplicateBarcodesSetNull} products\n`;
    output += `⭐ Barcodes Auto-Generated (Empty in Excel):     ${reportData.summary.autoGeneratedBarcodes} products\n`;
    output += `⚠️  Barcodes Modified (Cross-Brand Conflict):    ${reportData.summary.crossBrandBarcodeConflicts} products (added DUPL suffix)\n`;
    output += `📁 Empty ItemTypes Created:                      ${reportData.summary.emptyItemTypesCreated}\n\n`;
    ...
    if (reportData.emptyItemTypes.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `⭐ EMPTY ITEM TYPES CREATED - ${reportData.emptyItemTypes.length}\n`;
        output += '═'.repeat(120) + '\n';
    ...
    output += '═'.repeat(120) + '\n';
    output += `⭐ Products with AUTO-GENERATED CODE:             ${reportData.missingFields.noCost}\n`;
    output += `⭐ Products with AUTO-GENERATED BARCODE:          ${reportData.summary.autoGeneratedBarcodes}\n`; // ⭐ Changed    output += `⚠️  Products missing COST:                        ${reportData.missingFields.noCost}\n`;
    output += `ℹ️  Products with ZERO quantity:                  ${reportData.missingFields.zeroQuantity}\n\n`;

    // AUTO-GENERATED CODES
    if (reportData.autoGeneratedCodes.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `⭐ AUTO-GENERATED CODES - ${reportData.autoGeneratedCodes.length} PRODUCTS\n`;
        output += '═'.repeat(120) + '\n';
        ...
        reportData.autoGeneratedCodes.forEach((p, i) => {
            ...
            output += `  ✅ STATUS: INSERTED WITH AUTO-GENERATED CODE\n\n`;
        });
    }
    ...
    if (reportData.insertionErrors.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `❌ INSERTION ERRORS - ${reportData.insertionErrors.length} PRODUCTS FAILED TO INSERT\n`;
        output += '═'.repeat(120) + '\n';
        ...
        reportData.insertionErrors.forEach((err, i) => {
            ...
            if (err.conflictBrand) {
                output += `  ⚠️  CONFLICT WITH: ${err.conflictBrand} brand (same code already exists)\n`;
            }
            output += '\n';
        });
    }
    ...
    if (reportData.crossBrandBarcodeConflicts.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `⚠️ CROSS-BRAND BARCODE CONFLICTS - ${reportData.crossBrandBarcodeConflicts.length} FOUND\n`;
        output += '═'.repeat(120) + '\n';
        ...
        reportData.crossBrandBarcodeConflicts.forEach((conflict, i) => {
            ...
            output += `  ⚠️  DUPLICATE BARCODE FOUND IN: ${conflict.conflictBrand} brand\n`;
            output += `  Conflicting Product: ${conflict.conflictProduct}\n`;
            output += `  Location: ${conflict.subcategory} / ${conflict.itemType}\n`;
            output += `  ✅ ACTION TAKEN: Added "DUPL" suffix to barcode, product inserted\n\n`;
        });
    }
    ...
    if (reportData.duplicates.codes.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `❌ DUPLICATE CODES IN EXCEL - ${reportData.duplicates.codes.length} FOUND (ALL PRODUCTS REMOVED)\n`;
        output += '═'.repeat(120) + '\n';
        ...
        reportData.duplicates.codes.forEach(({ code, products }) => {
            output += `❌ CODE: ${code} (used ${products.length} times - ALL ${products.length} PRODUCTS REMOVED)\n`;
    ...
    if (reportData.duplicates.barcodes.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `⭐ DUPLICATE BARCODES IN EXCEL - ${reportData.duplicates.barcodes.length} FOUND\n`;
        output += '═'.repeat(120) + '\n';
        ...
        reportData.duplicates.barcodes.forEach(({ barcode, products }) => {
            output += `⭐ BARCODE: ${barcode} (used ${products.length} times)\n`;
            products.forEach((p, i) => {
                output += `  ${i + 1}. Row ${p.rowNumber}: ${p.name || '(NO NAME)'}\n`;
                output += `     Code: ${p.code}\n`;
                ...
                output += `     Location: ${p.subcategory} / ${p.itemType}\n`;
                output += `     Status: ✅ PRODUCT INSERTED WITHOUT BARCODE\n`;
            });
            output += '\n';
        });
    }
    ...
    if (reportData.skipped.noName.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `❌ SKIPPED - NO PRODUCT NAME (${reportData.skipped.noName.length})\n`;
        output += '═'.repeat(120) + '\n';
    ...
    if (reportData.autoGeneratedBarcodes && reportData.autoGeneratedBarcodes.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `⭐ PRODUCTS WITH AUTO-GENERATED BARCODE (${reportData.autoGeneratedBarcodes.length})\n`;
        output += '═'.repeat(120) + '\n';
        ...
        reportData.autoGeneratedBarcodes.slice(0, 30).forEach((p, i) => {
            ...
            output += `  Location: ${p.subcategory} / ${p.itemType}\n`;
            output += `  ✅ STATUS: INSERTED WITH AUTO-GENERATED BARCODE\n\n`;
        });
    ...
    if (reportData.detailedIssues.missingCost.length > 0) {
        output += '═'.repeat(120) + '\n';
        output += `⚠️ PRODUCTS WITH MISSING COST (${reportData.detailedIssues.missingCost.length})\n`;
        output += '═'.repeat(120) + '\n\n';
    ...
    console.log(`  ✅ Report saved: prisma/reports/${brandName}-Report.txt`);
}
...
async function processAllBrands() {
    ...
    for (const brand of BRANDS) {
        try {
            ...
            console.error(`\n❌ Error processing ${brand}:`, error.message);
        }
    }

    // Final Summary
    console.log('\n\n' + '═'.repeat(120));
    console.log('🎉 ALL BRANDS PROCESSED - FINAL SUMMARY');
    console.log('═'.repeat(120));
    ...
    console.log('═'.repeat(120));
    console.log('\n✅ All reports saved to: prisma/reports/\n');
