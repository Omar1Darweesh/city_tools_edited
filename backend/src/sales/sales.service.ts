...
export class SalesService {
  ...
  async getCustomerPendingPayments(customerId: number) {
    ...
    return invoices.map(inv => ({
      ...
      lines: inv.lines.map(line => ({
        ...
        productName: line.product?.nameEn || line.product?.nameAr || 'منتج غير معروف'
      }))
    }));
  }
  ...
  async findAll(params: {
    ...
    const itemsWithNumbers = items.map((sale) => ({
      ...
      // ADD THESE:
      totalRefunded: (sale.totalRefunded !== null && sale.totalRefunded !== undefined) ? Number(sale.totalRefunded) : 0,
      netRevenue: (sale.netRevenue !== null && sale.netRevenue !== undefined) ? Number(sale.netRevenue) : Number(sale.total),
