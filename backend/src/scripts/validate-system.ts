...
async function bootstrap() {
    ...
    try {
        ...
        if (Number(product.costAvg) !== 100) throw new Error('❌ Initial CostAvg invalid');
        ...
        if (Math.abs(Number(updatedProduct.costAvg) - 110) > 0.01) {
            throw new Error(`❌ CostAvg Calculation Failed. Expected 110, Got ${updatedProduct.costAvg}`);
        }
        ...
        if (Math.abs(Number(sale.costOfGoods) - expectedCost) > 0.01) {
            throw new Error(`❌ Sales CostOfGoods Failed. Expected ${expectedCost}, Got ${sale.costOfGoods}`);
        }
        if (Math.abs(Number(sale.netProfit) - expectedProfit) > 0.01) {
            throw new Error(`❌ Sales Profit Failed. Expected ${expectedProfit}, Got ${sale.netProfit}`);
        }
        ...
        const report = await reportsService.getDashboardSummary({ branchId: branch?.id });
